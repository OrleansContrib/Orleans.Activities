...
namespace Orleans.Activities
{
    ...
    /// <para>A Load with Runnable state typically means a Reload after a failure, when previously the workflow was persisted explicitly before a critical step.</para>
    /// </summary>
    ...
    [Description("Gets the IsReloaded property of the workflow. IsReloaded is true from a Load with Runnable state up to the first Save.\n" +
        "A Load with Runnable state typically means a Reload after a failure, when previously the workflow was persisted explicitly before a critical step.")]
